...
class _IdentityBlock(tf.keras.Model):
    ...
    def __init__(self, kernal_size, filters, stage, block, data_format):
        super(_IdentityBlock, self).__init__(name='')
        filters1, filter2, filters3 = filters

        conv_name_base = 'res' + str(stage) + block + '_branch'
        bn_name_base = 'res' + str(stage) + block + '_branch'
        bn_axis = 1 if data_format == 'channels_first' else 3
        self.conv2a = layers.Conv2D(filters1, (1, 1), data_format=data_format,
                                    name=conv_name_base + '2a')
        self.bn2a = layers.BatchNormalization(axis=bn_axis, name=bn_name_base + '2a')

        self.conv2b = layers.Conv2D(filter2, kernal_size, padding='same',
                                    data_format=data_format, name=conv_name_base + '2b')
        self.bn2b = layers.BatchNormalization(axis=bn_axis, name=bn_name_base + '2b')

        self.conv2c = layers.Conv2D(filters3, (1, 1), data_format=data_format,
...
class _ConvBlock(tf.keras.Model):
    ...
    def __init__(self, kernel_size, filters, stage, block, data_format, strides=(2, 2)):
        super(_ConvBlock).__init__(name='')
        ...
        self.conv2c = layers.Conv2D(filters3, (1, 1), strides=strides,
...
class ResNet50(tf.keras.Model):
    ...
    def __init__(self, data_format, name='', trainable=True,
                 include_top=True, pooling=None, classes=1000):
        ...
        def id_block(filters, stage, block):
            return _IdentityBlock(3, filters, stage=stage, block=block,
                                  data_format=data_format)

        self.conv1 = layers.Conv2D(64, (7, 7), strides=(2, 2), data_format=data_format,
